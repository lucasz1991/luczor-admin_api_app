...
class MemoryOrchestratorApiTest extends TestCase
{
    ...
    public function test_projection_control_calls_fit_below_job_and_redis_timeouts(): void
    {
        ...
        $this->assertLessThan($job->timeout, (int) config('luczor.cognee.timeout'));
    ...
    public function test_dataset_and_recall_are_isolated_per_tenant_and_user(): void
    {
        ...
        $this->assertStringContainsString("tenant:{$tenant->id}:user:{$first->id}:agent:planner:runs", $link->dataset);
        $this->assertStringNotContainsString(":user:{$second->id}:", $link->dataset);
    ...
    public function test_lost_add_response_is_replayed_deterministically_and_forgotten_after_source_delete(): void
    {
        ...
        try {
            ...
            // The durable `adding` intent and content snapshot survive.
        }
        $this->assertSame('adding', $outbox->fresh()->payload['phase']);
        $this->assertNotEmpty($outbox->fresh()->payload['content']);
    ...
    public function test_cached_launch_from_a_dead_cognee_instance_is_recovered_without_losing_its_run_identity(): void
    {
        Queue::fake();
        $user = User::factory()->create();
        $datasetId = '3e1e6f13-0360-4bb8-a14e-7ed8c9cb6ff9';
        ...
        $service->process($outbox->id);
        $this->assertSame($oldRunId, $outbox->fresh()->payload['pipeline_run_id']);
        $this->assertSame($oldInstanceId, $outbox->fresh()->payload['cognee_instance_id']);
        ...
        $this->assertSame($newRunId, $outbox->payload['pipeline_run_id']);
        $this->assertSame($newInstanceId, $outbox->payload['cognee_instance_id']);
        $this->assertSame(2, $cognee->launchRequests);
        $this->assertSame(1, $cognee->actualNewLaunches);
        $this->assertSame(1, $outbox->payload['recovery_generation']);
        Queue::assertNothingPushed();
    }
