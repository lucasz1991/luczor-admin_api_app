<x-ui.panel id="devices" class="mt-6 ">
            ...
            @forelse($devices as $device)
                <div class="rounded border border-slate-800 bg-slate-950/50 p-4">
                    ...
                    <div class="mt-3 flex flex-wrap gap-2">
                        <form method="POST" action="{{ route('dashboard.devices.debug.request', $device) }}">@csrf<input type="hidden" name="_dashboard_tool_group" value="operations"><x-ui.button variant="secondary" type="submit">Debug anfordern</x-ui.button></form>
    ...
    <section id="telemetry" class="mt-6 space-y-6">
        ...
        <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            @foreach ([
                ...
                <x-ui.panel class=" p-4"><div class="text-[10px] uppercase tracking-[.18em] text-slate-500">{{ $label }}</div><div class="mt-2 font-mono text-xl text-cyan-100">{{ $value }}</div></x-ui.panel>
            @endforeach
        </div>
        <x-ui.panel class=" overflow-x-auto">
