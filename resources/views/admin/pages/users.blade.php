<div class="grid gap-6 xl:grid-cols-[1.2fr_.8fr]">
                        ...
                        @unless($user->isAdmin())
                            <details class="mt-2"><summary>Bearbeiten</summary>
                                <form class="grid gap-2 py-3" method="POST" action="{{ route('dashboard.users.update', $user) }}">@csrf @method('PATCH')
                                    <label>Name<x-ui.input class="" name="name" value="{{ $user->name }}" required maxlength="160" aria-label="name" /></label>
                                    <label>E-Mail<x-ui.input class="" name="email" type="email" value="{{ $user->email }}" required aria-label="email" /></label>
    ...
    <x-ui.panel title="Benutzer anlegen">
        <form class="mt-4 grid gap-3" method="POST" action="{{ route('dashboard.users.store') }}">@csrf
            <label>Name<x-ui.input class="w-full" name="name" required maxlength="160" aria-label="name" /></label>
            <label>E-Mail<x-ui.input class="w-full" name="email" type="email" required aria-label="email" /></label>
            <label>Passwort<x-ui.input class="w-full" name="password" type="password" autocomplete="new-password" minlength="12" required aria-label="password" /></label>
            <label>Passwort bestätigen<x-ui.input class="w-full" name="password_confirmation" type="password" autocomplete="new-password" minlength="12" required aria-label="password confirmation" /></label>
            <x-ui.button class="" type="submit" variant="primary">Benutzer anlegen</x-ui.button>
        </form>
