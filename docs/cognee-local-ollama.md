...
- Cognee bleibt nur auf `127.0.0.1:${COGNEE_HOST_PORT:-8010}` erreichbar.
...
- Nur der opt-in Dienst `ollama-model-bootstrap` hat fuer den einmaligen Modelldownload ein ausgehendes Netz. Er gehoert zum deaktivierten Compose-Profil `model-bootstrap` und beendet sich nach dem Download.
...
Danach die persistenten Dienste und die Datenbankinitialisierung starten:

```bash
docker compose -f docker-compose.plesk-memory.yml up -d redis postgres
docker compose -f docker-compose.plesk-memory.yml run --rm cognee-db-init
docker compose -f docker-compose.plesk-memory.yml up -d ollama cognee
```
